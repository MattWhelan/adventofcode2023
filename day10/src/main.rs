...
use crate::Tile::{Connection, Empty, Start};
...
impl From<char> for Tile {
    fn from(value: char) -> Self {
        match value {
            '|' => Connection([North, South]),  // 2
            '-' => Connection([East, West]),    // 0
            'L' => Connection([North, East]),   // 1
            'J' => Connection([North, West]),   // -1
            '7' => Connection([South, West]),   // 1
            'F' => Connection([South, East]),   // -1
            '.' => Empty,
            'S' => Start,
...
impl FromStr for Map {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let grid = s.lines().map(|l| l.chars().map(|ch| ch.into()).collect()).collect();
        Ok(Map { grid })
    }
}
...
impl Map {
    fn find_start(&self) -> [usize; 2] {
        self.grid.iter().enumerate()
            .find_map(|(y, row)| row.iter().enumerate()
                .find(|(_, tile)| **tile == Start)
                .map(|(x, _)| [x, y]))
            .expect("No start!")
    }

    fn directions<'a>(&'a self, pos: &[usize; 2]) -> Box<dyn Iterator<Item=Direction> + 'a> {
        match self[pos] {
            Empty => Box::new(empty()),
            Start => {
                let pos = pos.clone();
                Box::new(
                    Direction::iter()
                        .filter(move |d| {
                            let off = d.offset();
                            let x = pos[0] as isize + off[0];
                            let y = pos[1] as isize + off[1];
                            if x >= 0 && y >= 0 && y < self.grid.len() as isize && x < self.grid[y as usize].len() as isize {
                                let adjacent = [x as usize, y as usize];
                                // check adjacent claims start as a neighbor
                                if self.neighbors(&adjacent).contains(&pos) {
                                    true
                                } else {
                                    false
                                }
    ...
    fn neighbors<'a>(&'a self, pos: &'a [usize; 2]) -> Box<dyn Iterator<Item=[usize; 2]> + 'a> {
        match self[pos] {
            Empty => Box::new(empty()),
            Start => {
                Box::new(
                    Direction::iter().map(|d| d.offset())
                        .filter_map(|off| {
                            let x = pos[0] as isize + off[0];
                            let y = pos[1] as isize + off[1];
                            if x >= 0 && y >= 0 && y < self.grid.len() as isize && x < self.grid[y as usize].len() as isize {
    ...
    fn distances(&self) -> HashMap<[usize; 2], usize> {
        let mut ret = HashMap::new();

        let mut to_visit: VecDeque<[usize; 2]> = VecDeque::new();

        let start = self.find_start();
        ret.insert(start.clone(), 0);

        to_visit.push_back(start);

        while let Some(pos) = to_visit.pop_front() {
            let d = ret[&pos];
            self.neighbors(&pos).for_each(|n| {
                let dist_entry = ret.entry(n.clone());
                match &dist_entry {
                    Entry::Occupied(_) => (),
                    Entry::Vacant(_) => {
                        dist_entry.or_insert(d+1);
    ...
    fn inside(&self) -> Vec<[usize; 2]> {
        // The set of points in the pipe loop
        let bounds: HashSet<_> = self.distances().keys().cloned().collect();
        (0..self.grid.len())
            .flat_map(|y| (0..self.grid[y].len()).map(move |x| [x,y]))
            .filter(|pos| {
                if !bounds.contains(pos) {
                    let y = pos[1];
                    // For each point to our west, count the connection-directions in the bounds
                    let hist: HashMap<Direction, usize> = (0..pos[0])
                        .map(|x| [x, y])
                        .filter(|p| bounds.contains(p))
                        .flat_map(|p| self.directions(&p))
                        .fold(HashMap::new(), |mut acc, d| {
                            *acc.entry(d).or_default() += 1;
                            acc
                        });
                    // A crossing is a matched north-south pair
                    let cross_count = hist.get(&North).unwrap_or(&0)
                        .min(hist.get(&South).unwrap_or(&0));
                    // If the cross count is odd, we're inside
                    cross_count % 2 == 1
                } else {
                    false
                }
...
fn main() {
    ...
    println!("Part 1: {}", map.distances().values().max().unwrap());
    let inside_points = map.inside();
    println!("Part 2: {}", inside_points.len());
}

const INPUT: &str = r"F.F77.F7F7FFJJ..77FF.--L777.-7777--F7F7FJFFF7FF|.F77FF|-7-|-F7-7-FF|FJ-FFJ7-F--F|-L--7L|-7FFFF---7-FJ.7J.F-7J.F|7-LF7L7|.F-L|7...LF--LL7F-77
