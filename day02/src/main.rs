...
impl Game {
    fn power(&self) -> u32 {
        let red = self.pulls.iter().map(|p| p.red).max().unwrap_or(0);
        let green = self.pulls.iter().map(|p| p.green).max().unwrap_or(0);
        let blue = self.pulls.iter().map(|p| p.blue).max().unwrap_or(0);
...
impl FromStr for Game {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        lazy_static! {
            ...
            static ref PULL_RE: Regex = Regex::new(r"(?:([^;]+)(?:; )?)").unwrap();
            static ref CUBE_RE: Regex = Regex::new(r"(?:(\d+) (\w+)(?:, )?)").unwrap();
        }

        if let Some(game_caps) = LINE_RE.captures(s) {
            let mut ret = Game::default();

            ret.id = game_caps[1].parse().unwrap();

            for pull_cap in PULL_RE.captures_iter(&game_caps[2]) {
                let mut pull = Pull::default();
                for cube_cap in CUBE_RE.captures_iter(&pull_cap[1]) {
                    let quantity = cube_cap[1].parse::<u32>().unwrap();
                    match &cube_cap[2] {
            ...
            Err(Error::msg("Line didn't match game"))
        }
    }
}

fn main() {
    let games: Vec<_> = INPUT.lines().map(|l| l.parse::<Game>().unwrap()).collect();

    let constraint = Pull { red: 12, green: 13, blue: 14 };
    let valid: Vec<_> = games.iter().filter(|g| g.pulls.iter().all(|p| p.fits(&constraint))).collect();
    println!("Part 1: {}", valid.iter().map(|g| g.id).sum::<u32>());

    println!("Part 2: {}", games.iter().map(|g| g.power()).sum::<u32>());
}

const INPUT: &str = r#"Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
