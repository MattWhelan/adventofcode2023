...
impl FromStr for Grid {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Grid{ grid: s.lines().map(|l| l.chars().collect()).collect() })
    }
}
...
impl Grid {
    fn neighbors(&self, index: &[usize; 2]) -> impl Iterator<Item = [usize; 2]> + '_ {
        let index = index.clone();
        (-1..=1).cartesian_product(-1..=1)
            .filter(|(x, y)| *x != 0 || *y != 0)
            .map(move |(dx, dy)| [(index[0] as isize) + (dx as isize), (index[1] as isize) + (dy as isize)])
            .filter(|[x, y]| {
                if *x > 0 && *y > 0 {
                    let x = *x as usize;
                    let y = *y as usize;
    ...
    fn points(&self) -> impl Iterator<Item = [usize; 2]> + '_ {
        (0..self.grid.len()).flat_map(|y| (0..self.grid[y].len()).map(move |x| [x, y]))
    }

    fn is_digit(&self, index: &[usize; 2]) -> bool {
        self[index].is_ascii_digit()
    }

    fn is_start_of_number(&self, index: &[usize; 2]) -> bool {
        self.is_digit(index) && (index[0] == 0 || !self.is_digit(&[index[0]-1, index[1]]))
    }

    fn is_symbol(&self, index: &[usize; 2]) -> bool {
        !self.is_digit(index) && self[index] != '.'
    }

    fn end_of_number(&self, index: &[usize; 2]) -> usize {
        let [x,y] = *index;
        (x..self.grid[index[1]].len())
            .take_while(|x_end| self.is_digit(&[*x_end, y]))
            .last()
            .expect("No number here!")
    }

    fn number_starting_at(&self, index: &[usize; 2]) -> u32 {
        let x_end = self.end_of_number(index);
        let s = (index[0]..=x_end).map(|xa| self[&[xa, index[1]]]).collect::<String>();

        s.parse().unwrap()
    }

    fn number_has_symbol(&self, index: &[usize; 2]) -> bool {
        let end_x = self.end_of_number(index);
        let [start_x,y] = *index;
        (start_x..=end_x).any(|x| self.neighbors(&[x, y]).any(|p| self.is_symbol(&p)))
    }

    fn get_symbols_for_number(&self, index: &[usize; 2]) -> impl Iterator<Item=[usize; 2]> + '_ {
        let end_x = self.end_of_number(index);
        let [start_x,y] = *index;
        (start_x..=end_x).flat_map(move |x| self.neighbors(&[x, y]).filter(|p| self.is_symbol(&p)))
    }

    // fn is_gear(&self, index: &[usize; 2]) -> bool {
    //     self[index] == '*' && self.neighbors(index).filter(|p| self.is_digit(p)).count() == 2
    // }

    fn numbers_with_symbols(&self) -> impl Iterator<Item=u32> + '_ {
        self.points()
            .filter(|p| self.is_start_of_number(p))
            .filter(|p| self.number_has_symbol(p))
            .map(|p| self.number_starting_at(&p))
    }

    fn gear_ratios(&self) -> impl Iterator<Item=u32> + '_ {
        let gear_number_indexes: Vec<_> = self.points()
            .filter(|p| self.is_start_of_number(p))
            .filter(|p| self.get_symbols_for_number(p).any(|p| self[&p] == '*'))
            .collect();

        let gear_numbers: HashMap<[usize; 2], Vec<u32>> = gear_number_indexes.iter()
            .fold(HashMap::new(), |mut acc, p| {
                if let Some(gear) = self.get_symbols_for_number(p)
                    .filter(|sym| self[sym] == '*')
                    .next() {
                    let nums = acc.entry(gear).or_default();
                    nums.push(self.number_starting_at(p))
                }

                acc
            });

        gear_numbers.into_iter()
...
fn main() {
    let schematic: Grid = INPUT.parse().unwrap();

    let part_numbers: Vec<u32> = schematic.numbers_with_symbols().collect();

    println!("Part 1: {}", part_numbers.iter().sum::<u32>());


    let gear_ratios_sum = schematic.gear_ratios().sum::<u32>();
